...
見積もり振り返り型タスク管理ツールのバックエンドAPI。タスクごとに「想定の大変さ」と「実際の大変さ」を記録し、見立ての精度を振り返れること、完了タスクの重さを日別に集計してコントリビューションカレンダー風に可視化できることを目的としている。
...
- フレームワークを使わない制約の中で、認証チェックを「ハンドラ関数を受け取って新しいハンドラ関数を返す」デコレータ関数として自前実装し、`main.go`側は`requireAuth(taskHandler.Create)`のように必要なエンドポイントだけラップする形にした
